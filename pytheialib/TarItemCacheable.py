...
class TarItemCacheable:
    ...
    def __init__(self, _archive, _tar, _node_temp_dir, _file):
        gdebug("# %s:%s(%s)" % (self.__class__, callee(), _file))

        self.archive = _archive  # Str
        self.tar = _tar  # <Tar>
        self.node_temp_dir = _node_temp_dir  # Str

        self._filepath = None  # property
        self.filepath_norm = None  # Str / managed by filepath's setter

        self._filename = None  # property
        self.filename = _file  # Use property's setter
        self.filename64 = None  # Str / managed by filename's setter
    ...
    def filename(self, _filename):
        ...
        Also sets 'filename64' and 'filepath'
        """
        # pylint: disable=E0202,E0102
        self._filename = _filename
        self.filename64 = str(base64.b64encode(bytes(self._filename, "utf-8")))
        self.filepath = _filename  # full path and base64 name
    ...
    def filepath(self, _filename):
        ...
        self.filepath_norm = os.path.join(
            self.node_temp_dir,
            str(base64.b64encode(bytes(self._filename, "utf-8"))).replace("'", "") + _ext,
        )
    ...
    def _uncompress_file(self):
        ...
        base64 encoding of the file basename + relative path inside cache, and
        ...
        _biobuff = self.tar.extract_file_tobuffer(self.filename)
    ...
    def uncompress_file_if_needed(self):
        ...
        if not self.filepath_norm:
            raise RuntimeError("filepath64 unset")
    ...
    def len(self):
        """
        Returns size in bytes of 'self.filepath64'
        """
        gdebug(f"# {self.__class__}:{callee()}")
    ...
    def close(self):
        ...
        Unlink file pointed by 'filepath64''
        """
